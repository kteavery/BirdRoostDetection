...
def get_heat_map_images(sess, model, i, j, height, stride, images,
...
def prediction_heat_map_on_bottleneck(sess, model, images, height, stride,
                                      bottleneck_tensor, resized_image_tensor):
    heat_map = []
    img_height = 299
    ...
    for i in range(height_width_range):
        for j in range(height_width_range):
            heat_map.append(
                get_heat_map_images(sess, model, i, j, height, stride, images,
                                    bottleneck_tensor, resized_image_tensor))
...
def create_heatmap_for_filename(image_path):
    ...
    with tf.Session(graph=graph) as sess:
        ...
        image_paths = [image_path.format(radar_field) for radar_field in
                       aggregate_train.dual_pol_fields]
        ...
        heat_maps.append(
            prediction_heat_map_on_bottleneck(sess, model, images, 48, 16,
                                              bottleneck_tensor,
                                              resized_image_tensor))
        heat_maps.append(
            prediction_heat_map_on_bottleneck(sess, model, images, 60, 30,
                                              bottleneck_tensor,
                                              resized_image_tensor))
        heat_maps.append(
            prediction_heat_map_on_bottleneck(sess, model, images, 80, 20,
                                              bottleneck_tensor,
                                              resized_image_tensor))
