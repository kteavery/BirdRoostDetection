...
def maybe_download_and_extract(data_url):
    ...
    if not os.path.exists(filepath):
        ...
        print(
            'Not extracting or downloading files, model already present in '
            'disk')
...
def create_bottleneck_file(bottleneck_path, image_lists, label_name, index,
    ...
    image_path = get_image_path(image_lists, label_name, index, category)
...
def get_or_create_bottleneck(sess, image_lists, label_name, index, image_dir,
    ...
    if did_hit_error:
        ...
        with open(bottleneck_path, 'r') as bottleneck_file:
            bottleneck_string = bottleneck_file.read()
        # Allow exceptions to propagate here, since they shouldn't happen
        # after a
        # fresh creation
        bottleneck_values = [float(x) for x in bottleneck_string.split(',')]
    return bottleneck_values
        ...
        for category in ['training', 'testing', 'validation']:
            ...
            for index, unused_base_name in enumerate(category_list):
                ...
                if how_many_bottlenecks % 100 == 0:
                    tf.logging.info(
                        str(
                            how_many_bottlenecks) + ' bottleneck files '
                                                    'created.')
...
def add_input_distortions(flip_left_right, random_crop, random_scale,
    ...
    cropping is performed. If the value is 50%, then the crop box will be
    half the
    ...
    input and no scaling is applied. If it's 50%, then the bounding box will
    be in
...
def add_final_retrain_ops(class_count, final_tensor_name, bottleneck_tensor,
    ...
    function
    adds the right operations to the graph, along with some variables to hold
    the
    ...
    Args:
      ...
      quantize_layer: Boolean, specifying whether the newly added layer
      should be
          instrumented for quantized.
      is_training: Boolean, specifying whether the newly add layer is for
      training
          or eval.
    Returns:
...
def create_model_info(architecture):
    ...
    retrained using transfer learning, and this function translates from the
    name
...
def main(_):
    ...
    with tf.Session(graph=graph) as sess:
        ...
        run_final_eval(sess, model_info, class_count, image_lists,
                       jpeg_data_tensor,
                       decoded_image_tensor, resized_image_tensor,
                       bottleneck_tensor)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
      ...
      pixel input images, while 'mobilenet_0.25_128_quantized' will choose a 
      much
      smaller and less accurate model, taking 128x128 images, and instrumented
      for eventual quantization via TensorFlow Lite.
      See https://research.googleblog.com/2017/06/mobilenets-open-source
      -models-for.html
      for more information on Mobilenet.\
      """)
    parser.add_argument(
